...
# ---- Web panel + AI + static IP config (v9.2) ----
# The panel is a separate process (panel_app.py). This bot only generates
# per-VPS panel credentials and shows the link in the creation DM.
PANEL_URL = os.getenv('PANEL_URL', 'http://127.0.0.1:8080')
# Never hardcode this key. Set it as an environment variable on the host
# (e.g. in the systemd unit or a .env file) instead.
GEMINI_API_KEY = os.getenv('GEMINI_API_KEY', '')
GEMINI_MODEL = os.getenv('GEMINI_MODEL', 'gemini-2.0-flash')
# Static IP support: base subnet used for the LXD bridge that hands out
# fixed addresses per container (see assign_static_ip()).
STATIC_IP_SUBNET = os.getenv('STATIC_IP_SUBNET', '10.20.30.0/24')
STATIC_IP_BRIDGE = os.getenv('STATIC_IP_BRIDGE', 'lxdbr-static')
STATIC_IP_ENABLED = os.getenv('STATIC_IP_ENABLED', 'false').lower() == 'true'
...
def init_db():
    ...
    if 'panel_token' not in columns:
        cur.execute("ALTER TABLE vps ADD COLUMN panel_token TEXT DEFAULT ''")
...
def save_vps_data():
    ...
    for user_id, vps_list in vps_data.items():
        for vps in vps_list:
            ...
            panel_token = vps.get('panel_token', '')
            static_ip = vps.get('static_ip', '')
            if 'id' not in vps or vps['id'] is None:
                cur.execute('''INSERT INTO vps (user_id, node_id, container_name, ram, cpu, storage, config, os_version, status, suspended, whitelisted, created_at, shared_with, suspension_history, root_password, panel_token, static_ip)
                               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                            (user_id, node_id, vps['container_name'], vps['ram'], vps['cpu'], vps['storage'], vps['config'],
                             os_ver, vps['status'], suspended_int, whitelisted_int,
                             created_at, shared_json, history_json, root_password, panel_token, static_ip))
                vps['id'] = cur.lastrowid
            else:
                cur.execute('''UPDATE vps SET user_id = ?, node_id = ?, container_name = ?, ram = ?, cpu = ?, storage = ?, config = ?, os_version = ?, status = ?, suspended = ?, whitelisted = ?, shared_with = ?, suspension_history = ?, root_password = ?, panel_token = ?, static_ip = ?
                               WHERE id = ?''',
                            (user_id, node_id, vps['container_name'], vps['ram'], vps['cpu'], vps['storage'], vps['config'],
                             os_ver, vps['status'], suspended_int, whitelisted_int, shared_json, history_json, root_password, panel_token, static_ip, vps['id']))
    conn.commit()
    conn.close()
...
def generate_panel_token(length: int = 40) -> str:
    """Generate a high-entropy token for web-panel login (VPS ID + token)."""
    return secrets.token_urlsafe(length)

def _used_static_ips() -> set:
    conn = get_db()
    cur = conn.cursor()
...
def _next_static_ip() -> Optional[str]:
    """Pick the next free host address inside STATIC_IP_SUBNET (skips .0/.1/.255)."""
    import ipaddress
    network = ipaddress.ip_network(STATIC_IP_SUBNET, strict=False)
    used = _used_static_ips()
    for host in network.hosts():
        ip_str = str(host)
        if ip_str.endswith('.1'):
            continue  # reserved for the bridge gateway
        if ip_str not in used:
            return ip_str
    return None

async def assign_static_ip(container_name: str, node_id: int) -> Optional[str]:
    """
    Assigns a fixed IP to a container on the static-IP bridge, so the address
    never changes across reboots/restarts. Requires STATIC_IP_ENABLED=true and
    the STATIC_IP_BRIDGE network to already exist (the installer sets this up).
    No-ops (returns None) when static IPs aren't enabled for this deployment.
    """
    if not STATIC_IP_ENABLED:
        return None
    ip = _next_static_ip()
    if not ip:
        logger.warning("Static IP pool exhausted; falling back to DHCP.")
        return None
    try:
        await execute_lxc(
            container_name,
            f"config device override {container_name} eth0 "
            f"network={STATIC_IP_BRIDGE} ipv4.address={ip}",
            node_id=node_id, timeout=60
        )
    except Exception:
        # Device may not exist yet (fresh container) - add it fresh instead.
        try:
            await execute_lxc(
                container_name,
                f"config device add {container_name} eth0 nic "
                f"network={STATIC_IP_BRIDGE} ipv4.address={ip}",
                node_id=node_id, timeout=60
            )
        except Exception as e:
            logger.error(f"Failed to assign static IP {ip} to {container_name}: {e}")
            return None
    return ip

# ---------------------------------------------------------------------------
# Gemini AI helper (used by !ask). Requires GEMINI_API_KEY to be set as an
# environment variable — never hardcode a key in this file or commit one.
# ---------------------------------------------------------------------------
async def ask_gemini(prompt: str) -> str:
    if not GEMINI_API_KEY:
        return "AI features are disabled: no GEMINI_API_KEY set on the server."
    url = (
    ...
    try:
        ...
        if not candidates:
            return "Gemini returned no response (it may have blocked the prompt)."
        parts = candidates[0].get("content", {}).get("parts", [])
        return "".join(p.get("text", "") for p in parts) or "Gemini returned an empty response."
    except Exception as e:
...
class OSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        try:
            ...
            static_ip = await assign_static_ip(container_name, self.node_id)
            panel_token = generate_panel_token()
            ...
            if static_ip:
                add_field(dm_embed, "🌐 Static IP", f"`{static_ip}` (fixed — won't change on restart)", False)
            add_field(dm_embed, "🖥️ Web Panel Login", f"**Panel:** {PANEL_URL}\n**VPS ID:** `{panel_vps_id}`\n**Token:** `{panel_token}`\n\nUse these to log in to the panel for live status, start/stop/restart/reinstall, port management, and a live browser terminal.", False)
            add_field(dm_embed, "Management", f"• Use `{PREFIX}manage` to start/stop/reinstall your VPS\n• Use `{PREFIX}manage` → SSH for terminal access\n• Or use the web panel above for the same controls plus a live terminal\n• Contact admin for upgrades or issues", False)
            add_field(dm_embed, "Important Notes", "• Full root access via SSH\n• Docker-ready with nesting and privileged mode\n• Back up your data regularly\n• Change your root password after first login\n• Keep your panel token private — anyone with it can control this VPS", False)
            try:
...
class ReinstallOSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Static IP and panel token persist across reinstalls; only create
            # them if this VPS predates the v9.2 upgrade.
            static_ip = target_vps.get("static_ip") or await assign_static_ip(self.container_name, self.node_id) or ''
            if not target_vps.get("panel_token"):
                target_vps["panel_token"] = generate_panel_token()
            ...
            target_vps["static_ip"] = static_ip
            ...
            try:
                ...
                if pinggy_address:
                    ...
                    add_field(dm_embed, "🔑 Root Password", f"`{root_password}`\n⚠️ SSH tunnel setup failed — use `{PREFIX}manage` → 🔌 Reconnect Tunnel to try again.", False)
                if static_ip:
                    add_field(dm_embed, "🌐 Static IP", f"`{static_ip}` (unchanged)", False)
                add_field(dm_embed, "🖥️ Web Panel", f"Your VPS ID and panel token are unchanged — log in at {PANEL_URL} with the same credentials.", False)
        ...
        self.command_categories = {
            ...
            "bot": {
                ...
                "commands": [
                    ...
                    (f"{PREFIX}ask <question>", "Ask the built-in AI assistant"),
                    (f"{PREFIX}set-status <type> <name>", "Set bot status (Admin only)")
                ]
            },
            "admin": {
...
async def ask_ai(ctx, *, question: str = None):
    """Ask the bot's built-in AI assistant a question (VPS/Linux/Docker help, etc.)."""
    if not question:
        await ctx.send(embed=create_error_embed("Usage", f"`{PREFIX}ask <your question>`"))
        return
    async with ctx.typing():
        system_context = (
            f"You are the AI assistant built into {SERVER_NAME}, a Discord VPS hosting bot. "
            "Answer concisely and practically. If asked about VPS management, prefer pointing "
            f"users to `{PREFIX}manage` or the web panel rather than making promises the bot can't keep."
        )
        answer = await ask_gemini(f"{system_context}\n\nUser question: {question}")
    embed = create_info_embed("🤖 AI Assistant", truncate_text(answer, 4000))
    await ctx.send(embed=embed)
